...
import { _ as _debug, t as toArray, e as DiffMap, D as DependenciesTypeShortMap, u as updateTargetVersion, g as getVersionOfRange, f as getPrefixedVersion, C as CheckPackages, w as writePackage, c as loadPackages, h as getPackageData } from './shared/taze.a5698047.mjs';
...
const FIG_CHECK = picocolorsExports.green("\u25C9");
const FIG_UNCHECK = picocolorsExports.gray("\u25CC");
const FIG_POINTER = picocolorsExports.cyan("\u276F ");
const FIG_NO_POINTER = "  ";
const FIG_BLOCK = picocolorsExports.bold(picocolorsExports.dim(picocolorsExports.gray("\u2503")));
...
function colorizeVersionDiff(from, to, hightlightRange = true) {
  ...
  return picocolorsExports[leadingColor](leadingWildcard) + partsToColor.slice(0, i).join(".") + middot + picocolorsExports[color](partsToColor.slice(i).join(".")).trim();
}
...
function createMultiProgresBar() {
  return new cliProgress.MultiBar({
    ...
    hideCursor: true,
    format: `{type} {bar} {value}/{total} ${picocolorsExports.gray("{name}")}`,
    linewrap: false,
    barsize: 40
...
function timeDifference(from, to = +/* @__PURE__ */ new Date()) {
  ...
  if (elapsed < msPerDay)
    return picocolorsExports.gray("\u2A7D1d");
  else if (elapsed < msPerMonth)
    return picocolorsExports.green(`~${Math.round(elapsed / msPerDay)}d`);
  else if (elapsed < msPerYear)
    return picocolorsExports.yellow(`~${Math.round(elapsed / msPerMonth)}mo`);
  else
    return picocolorsExports.red(`~${+(elapsed / msPerYear).toFixed(1)}y`);
}
...
function renderChange(change, interactive) {
  ...
  return [
    `${pre} ${update ? change.name : picocolorsExports.gray(change.name)}`,
    picocolorsExports.gray(DependenciesTypeShortMap[change.source]),
    timeDifference(change.currentVersionTime),
    picocolorsExports.gray(change.currentVersion),
    update ? picocolorsExports.dim(picocolorsExports.gray("\u2192")) : "",
    update ? colorizeVersionDiff(change.currentVersion, change.targetVersion) : picocolorsExports.gray(picocolorsExports.strikethrough(change.targetVersion)),
    update ? timeDifference(change.targetVersionTime) : "",
    change.latestVersionAvailable && semver.minVersion(change.targetVersion).toString() !== change.latestVersionAvailable ? picocolorsExports.dim(picocolorsExports.magenta(`(${change.latestVersionAvailable} available)`)) : ""
  ];
}
function renderChanges(pkg, options, interactive) {
  ...
  if (changes.length) {
    ...
    const diffEntries = Object.keys(diffCounts).length ? Object.entries(diffCounts).map(([key, value]) => `${picocolorsExports.yellow(value)} ${key}`).join(", ") : picocolorsExports.dim("no change");
    lines.push(
      // c.dim(c.gray(filepath)),
      `${picocolorsExports.cyan(pkg.name ?? "\u203A")} ${picocolorsExports.dim("-")} ${diffEntries}`,
      ""
    );
    ...
    lines.push(`${picocolorsExports.cyan(pkg.name)} ${picocolorsExports.dim(filepath)}`);
    lines.push();
    lines.push(picocolorsExports.gray("  \u2713 up to date"));
  }
...
function renderResolveError(dep) {
  ...
  if (dep.resolveError === "404") {
    lines.push(picocolorsExports.red(`> ${picocolorsExports.underline(dep.name)} not found`));
  } else if (dep.resolveError === "invalid_range") ; else {
    lines.push(picocolorsExports.red(`> ${picocolorsExports.underline(dep.name)} unknown error`));
    lines.push(picocolorsExports.red(dep.resolveError.toString()));
  }
  return lines;
}
...
async function promptInteractive(pkgs, options) {
  ...
  function createListRenderer() {
    ...
    return {
      render() {
        const Y = (v) => picocolorsExports.bold(picocolorsExports.green(v));
        console.clear();
        console.log(`${FIG_BLOCK} ${picocolorsExports.gray(`${Y("\u2191\u2193")} to select, ${Y("space")} to toggle, ${Y("\u2192")} to change version`)}`);
        console.log(`${FIG_BLOCK} ${picocolorsExports.gray(`${Y("enter")} to confirm, ${Y("esc")} to cancel`)}`);
        console.log();
        const lines = [];
        pkgs.forEach((pkg) => {
  ...
  function createVersionSelectRender(dep) {
    ...
    return {
      render() {
        console.clear();
        console.log(`${FIG_BLOCK} ${picocolorsExports.gray(`Select a version for ${picocolorsExports.green(picocolorsExports.bold(dep.name))}${picocolorsExports.gray(` (current ${dep.currentVersion})`)}`)}`);
        console.log();
        console.log(
          formatTable(versions.map((v, idx) => {
            return [
              (index === idx ? FIG_POINTER : FIG_NO_POINTER) + (index === idx ? v.name : picocolorsExports.gray(v.name)),
              timeDifference(dep.currentVersionTime),
              picocolorsExports.gray(dep.currentVersion),
              picocolorsExports.dim(picocolorsExports.gray("\u2192")),
...
async function check(options) {
  ...
  await CheckPackages(options, {
    afterPackagesLoaded(pkgs) {
      packagesBar = options.recursive && pkgs.length ? bars?.create(pkgs.length, 0, { type: picocolorsExports.cyan("pkg"), name: picocolorsExports.cyan(pkgs[0].name) }) : void 0;
    },
    beforePackageStart(pkg) {
      packagesBar?.increment(0, { name: picocolorsExports.cyan(pkg.name) });
      depBar?.start(pkg.deps.length, 0, { type: picocolorsExports.green("dep") });
    },
    beforePackageWrite() {
      return false;
    },
  ...
  if (!hasChanges) {
    console.log(picocolorsExports.green("dependencies are already up-to-date"));
    return exitCode;
  }
  ...
  if (!options.all) {
    ...
    if (last === 1)
      console.log(picocolorsExports.green("dependencies are already up-to-date in one package\n"));
    else if (last > 0)
      console.log(picocolorsExports.green(`dependencies are already up-to-date in ${last} packages
`));
  }
  if (errLines.length) {
    console.error(picocolorsExports.inverse(picocolorsExports.red(picocolorsExports.bold(" ERROR "))));
    console.error();
    ...
    options.write = await prompts([
      {
        ...
        type: "confirm",
        initial: true,
        message: picocolorsExports.green("write to package.json")
      }
    ]).then((r) => r.write);
  }
  if (options.write) {
  ...
  if (!options.write) {
    ...
    if (options.mode === "default")
      console.log(`Run ${picocolorsExports.cyan("taze major")} to check major updates`);
    if (hasChanges) {
      if (options.failOnOutdated)
        exitCode = 1;
      console.log(`Add ${picocolorsExports.green("-w")} to write to package.json`);
    }
    console.log();
  } else if (hasChanges) {
    if (!options.install && !options.update && !options.interactive) {
      console.log(
        picocolorsExports.yellow(`\u2139 changes written to package.json, run ${picocolorsExports.cyan("npm i")} to install updates.`)
      );
    }
    if (options.install || options.update || options.interactive)
      console.log(picocolorsExports.yellow("\u2139 changes written to package.json"));
      ...
      options.install = await prompts([
        {
          ...
          type: "confirm",
          initial: true,
          message: picocolorsExports.green("install now")
        }
      ]).then((r) => r.install);
    }
    if (options.install) {
      console.log(picocolorsExports.magenta("installing..."));
      console.log();
      await run(parseNi, []);
    }
    if (options.update) {
      console.log(picocolorsExports.magenta("updating..."));
      console.log();
...
async function usage(options) {
  ...
  const resolveUsages = await CheckUsages(options, {
    onLoaded(usages) {
      depBar = bars.create(usages.length, 0, { type: picocolorsExports.green("deps") });
    },
  ...
  for (const { name, versionMap, latest } of resolveUsages) {
    ...
    if (versions.length > 1) {
      ...
      if (options.detail) {
        logger.log();
        logger.row(
          `${picocolorsExports.green(name)} ${picocolorsExports.gray(`\xB7 ${versions.length} versions \xB7 latest: ${picocolorsExports.blue(latest)}`)}`
        );
        const pad = Math.max(8, ...Object.keys(versionMap).map((i) => i.length)) + 2;
        for (const [version, pkgs] of Object.entries(versionMap)) {
          const lines = wrapJoin(pkgs.map((p) => p.name), picocolorsExports.gray(", "), 80);
          lines.forEach((line, i) => {
            if (i === 0)
              logger.log(`${visualPadStart(picocolorsExports.gray(colorizeVersionDiff(latest || version, version, false)), pad, " ")}  ${line}`);
            else
        ...
        logger.row(
          picocolorsExports.green(name),
          picocolorsExports.gray(`${picocolorsExports.cyan(packagesCount.toString())} in use / ${picocolorsExports[color](versions.length.toString())} versions`),
          versions.map((v) => picocolorsExports.gray(colorizeVersionDiff(latest || v, v, false))).join(picocolorsExports.gray(", ")),
          picocolorsExports.gray("\u2192"),
          latest
        );
      }
    }
...
yargs(hideBin(process.argv)).scriptName("taze").usage("$0 [args]").command(
  ...
  (args) => {
    ...
    }).help().demandOption("recursive", picocolorsExports.yellow("Please add -r to analysis usages"));
  },
